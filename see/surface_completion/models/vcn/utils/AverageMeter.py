    ...
    def avg(self, idx=None):
        if idx is None:
            if self.items is None:
                ...
                return [self._sum[i] / self._count[i] if self._count[i] != 0 else -1 for i in range(self.n_items)]
        else:
            return self._sum[idx] / self._count[idx] if self._count[idx] != 0 else -1