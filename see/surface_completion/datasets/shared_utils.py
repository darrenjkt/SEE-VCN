...
def populate_gtboxes(sample_infos, dataset_name, classes, add_ground_lift=False, ground_lift_height=0.1):
    if dataset_name == 'nuscenes':
        zip_infos = zip(sample_infos['gt_boxes'], sample_infos['gt_names'], sample_infos['num_lidar_pts'])
    elif dataset_name in ['kitti', 'waymo','custom']:
    ...
    for idx, gt_anno in enumerate(zip_infos):
        ...
        if bbox_corners != None:
            if add_ground_lift:
                bbox_corners.center = bbox_corners.center + [0,0,ground_lift_height/2] # Add to box centroid z-axis to get rid of the ground plane for slopes
                bbox_corners.extent = bbox_corners.extent + [0,0,-ground_lift_height]
...
def get_pts_in_mask(dataset, instances, imgfov, shrink_percentage=0, use_bbox=False, append_mask_info=False):
    """
    ...
    list_instance_pts_uv, list_instance_pc_cam_xyz, pointcloud = [], [], []
    instance_dict_list = []
    for instance_orig in instances:
        ...
        if use_bbox:
            ...
            seg_mask = dataset.annToMask(instance) # Once new htc exported, we won't have to do this
            mask = seg_mask   
            instance['bin_mask'] = mask
        ...
        if instance_pc_lidar.shape[0] == 0:
            continue
        ...
        if append_mask_info:
            # We can attach class label and segmentation mask length
            # If the object is split into more than 2 parts then we can increase the clustering value
    ...
    # img_uv: point cloud uv coordinates in image frame
    # cam_xyz: point cloud xyz coordinates in camera frame
    # lidar_xyz: point cloud xyz coordinates in lidar frame
    # img_labels: coco annotation format (mask polygon, mask binary, bbox etc.)
    instance_pts = {"img_uv": list_instance_pts_uv,
                    "cam_xyz":list_instance_pc_cam_xyz,
                    "lidar_xyz": pointcloud,
                    "img_labels": instance_dict_list}
    return instance_pts
...
def draw_lidar_on_image(projected_points, img, instances=None, 
                        ...
                        shrink_percentage=0, display=True, instance_mask=False, draw_bbox=False):
    ''' 
...
def shrink_instance_masks(seg_masks, shrink_percentage):
    ...
    for seg in seg_masks:
        ...
        if isinstance(resized_poly, geometry.MultiPolygon):
            resized_poly = list(resized_poly.geoms)
...
def db_scan(pcd, eps, min_pts=3, return_largest_cluster=False):
    labels = np.array(pcd.cluster_dbscan(eps=eps, min_points=min_pts))
    y = np.bincount(labels[labels >= 0])
    
    if return_largest_cluster:   
        if len(y) > 0:    
            cluster_indices = np.argwhere(labels == np.argmax(y))
            return pcd.select_by_index(cluster_indices)
        else:
            return pcd
    else:
        clusters = [pcd.select_by_index(np.argwhere(labels == indices)) for indices in range(len(y))]
        return clusters