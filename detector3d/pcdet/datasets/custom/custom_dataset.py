...
class CustomDataset(DatasetTemplate):
    ...
    def include_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading SCCustomDataset dataset')
        custom_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for SCCustomDataset dataset: %d' % (len(self.infos)))
    ...
    def get_image_shape(self, idx):
        img_file = self.root_split_path / 'image' / 'front' / ('%s.jpg' % idx)
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pcd = custom_dataset_utils.convert_to_o3dpcd(self.get_lidar(sample_idx))
            num_points_in_gt = []
            for i in range(len(anno['gt_boxes_lidar'])):
                gt_box = anno['gt_boxes_lidar'][i]
                try:
                    o3dbox = custom_dataset_utils.get_o3dbox(gt_box)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        if self.dataset_cfg.get('MIN_POINTS_OF_GT', False):
            print(f'Removing gt with points less than {self.dataset_cfg.MIN_POINTS_OF_GT}')
            eval_gt_annos = []
            for info in self.infos:
                annos = info['annos']
                minpts_mask = (annos['num_points_in_gt'] >= self.dataset_cfg.MIN_POINTS_OF_GT)
...
def create_custom_dataset_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = SCCustomDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    ...
    # TODO: Change these to be taken in from args. 
    custom_infos = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(test_filedir / 'custom_infos_test.pkl', 'wb') as f:
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_dataset_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
        dataset_cfg = EasyDict(yaml.load(open(sys.argv[2])))
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_custom_dataset_infos(
